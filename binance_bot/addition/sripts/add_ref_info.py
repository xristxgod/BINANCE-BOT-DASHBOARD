...
def insert_info(info: typing.Dict, user_id: int):
    ...
    for i in info.values():
        if decimals.create_decimal(i) > 0:
            summa_ref_profit += decimals.create_decimal(i)
    if summa_ref_profit > 0:
        post_sql_from_db(f"UPDATE user_model SET budget = budget + {summa_ref_profit} WHERE id = {user_id};")
    return True
